...
![](https://github.com/Higourang/DevOpProjects/blob/master/SimpeDevOpsProjects/Project-4.MD)
...
1. Create *Jenkins job*, Fill the following details,
   - *Source Code Management:*
      - Repository : https://github.com/Higourang/hello-world.git
   ...
   - *Post Steps*
     ...
     - *Send files or execute commands over SSH*
       ...
       - Exec Command: 
	      ...
	      - `docker tag DevOps_demo DevOps/DevOps_demo`
          - `docker push DevOps/DevOps_demo`
          - `docker rmi DevOps_demo DevOps/DevOps_demo`
              
1. Login to Docker host and check images and containers. (no images and containers)

1. login to docker hub and check. shouldn't find images with for DevOps_demo

1. Execute Jenkins job

1. check images in Docker hub. Now you could able to see new images pushed to gourang Docker_Hub
     ...
     - hosts: web-servers
       ...
       tasks:
        - name: stop previous version docker
          shell: docker stop DevOps_demo
        - name: remove stopped container
          shell: docker rm -f DevOps_demo	  
        - name: remove docker images
          shell: docker image rm -f DevOps/DevOps_demo
          
        - name: create docker image
          shell: docker run -d --name DevOps_demo -p 8090:8080 DevOps/DevOps_demo
...
1. Create Jenkins job 
   ...
   - *Send files or execute commands over SSH*
     ...
     - Remote directory	: `//opt//docker`
        ...
        - `docker tag $JOB_NAME:v1.$BUILD_ID DevOps/$JOB_NAME:v1.$BUILD_ID`
        - `docker tag $JOB_NAME:v1.$BUILD_ID DevOps/$JOB_NAME:latest`
        - `docker push DevOps/$JOB_NAME:v1.$BUILD_ID`
        - `docker push DevOps/$JOB_NAME:latest`
        - `docker rmi $JOB_NAME:v1.$BUILD_ID DevOps/$JOB_NAME:v1.$BUILD_ID` 
        - `DevOps/$JOB_NAME:latest`
##### References
